...
- **`resolveToday` in `src/lib/dates.ts` is the only place a timezone is interpreted.** The zone is an argument, not a second implementation: `resolveToday(tz)` for display, `resolveToday("UTC")` for a policy-compared column (`src/lib/db/medications.ts` → `todayUtc`). Anything reaching for `new Date().toISOString().slice(0, 10)` or a fresh `Intl.DateTimeFormat` is re-deriving a date this function already owns.
- **There are two "todays", and they may differ by one calendar day. That is intended.** A date resolved for **user-facing classification** — the Upcoming/Past split on `/visits` and the hints reading from it — uses the user's stored zone. A date an **RLS policy compares against Postgres `current_date`** is resolved in **UTC**, because `current_date` on Supabase is UTC: at 22:00 in UTC-8 a local "today" is yesterday in UTC, so the row just written already fails `effective_date >= current_date` and the dosage cannot be corrected. Do not reconcile the divergence by routing policy-compared writes through the user's zone — that reintroduces the bug S-02's migration removed. It is **column-scoped, not a blanket rule**.
