...
- **A date column an RLS policy compares against Postgres `current_date` is resolved in UTC** — today `dosage_changes.effective_date`, and `supply_events.occurred_on` by symmetry. `current_date` on Supabase is UTC, so a date taken from the visitor's clock disagrees with it for part of every day: at 22:00 in UTC-8 a local "today" is yesterday in UTC, and the row just written already fails `effective_date >= current_date`, leaving the dosage uncorrectable. The server derives it; no route or island may send one. This is **column-scoped, not a blanket rule**: a date resolved for user-facing classification (an Upcoming/Past split, say) belongs in the user's own stored zone. The two "todays" may differ by one calendar day, and that is intended.
