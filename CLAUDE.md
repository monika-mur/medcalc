...
- **`src/components/form/` is ours to edit; `src/components/ui/` is not.** Shared form controls (`FormField`, `SelectField`) live in `src/components/form/`. Files under `src/components/ui/` are generated by shadcn — needing to edit one to change a colour means the token is wrong (see _Design conventions_).
...
- **Domain dates are `YYYY-MM-DD` strings, compared as strings.** That format is simultaneously what `<input type="date">` reads and writes, what Postgres uses on the wire for a `date` column, and what `Intl.DateTimeFormat("en-CA")` produces. Introducing a `Date` object onto this path is how an off-by-one-day bug gets in.
- **There are two "todays", and they may differ by one calendar day. That is intended.** A date resolved for **user-facing classification** — the Upcoming/Past split on `/visits` and the hints reading from it — uses the user's stored zone: `resolveToday(tz)` in `src/lib/dates.ts`. A date an **RLS policy compares against Postgres `current_date`** is resolved in **UTC**, because `current_date` on Supabase is UTC and a zone behind it would write yesterday and fail the policy. Do not reconcile the divergence by routing policy-compared writes through the user's zone.
- **Resolve it once, server-side, and pass it down as a prop.** An island must never call `new Date()` for this — a second "today" is exactly what lets a grouping and its own hint disagree about the same row.
- **The stored zone is hostile input, and UTC is its normal fallback, not a degraded one.** `user_metadata` is user-writable via `auth.updateUser({ data })`, and an invalid zone throws `RangeError` at `Intl` construction, so `resolveToday` catches and falls back. The zone is stamped only by the signup page's inline script and signin never backfills it, so every account created without JavaScript resolves through the fallback. Callers must narrow `user_metadata.timezone` to `string | undefined` first: it is typed `any`, and `strictTypeChecked` reports `no-unsafe-argument`.
