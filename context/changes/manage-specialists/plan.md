...
- [ ] 2.5 Sign-in and sign-up render correctly and both still submit successfully
- [ ] 2.6 A real signup still stores the browser timezone in `raw_user_meta_data`
- [ ] 2.7 Server-side error display still works on a duplicate-email submit
- [ ] 2.8 Both auth screens usable at 320 px with no horizontal scrolling
- [ ] 2.9 Every existing screen renders white/slate with green only on primary actions, links, and focus rings
- [ ] 2.10 Contrast passes AA on the auth screens and dashboard (text ≥ 4.5:1, UI ≥ 3:1)
- [ ] 2.11 Keyboard focus is visible on every interactive element
- [ ] 2.12 Replaced landing page at `/` renders, its sign-in/sign-up buttons work, and the tab title is no longer "10x Astro Starter"
