...
| S-05 | mid-supply-dosage-change   | schedule a future dosage change and see the recalculated status                            | S-02, S-04    | FR-006, US-02                            | done     | [#6](https://github.com/monika-mur/medcalc/issues/6) |
...
| ---- | ------------------------ | ---------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| S-05 | mid-supply-dosage-change | Schedule a future dosage change (new daily amount, effective date); the dashboard recalculates the supply-end date and status segmentally | 2026-09-14 | [#6](https://github.com/monika-mur/medcalc/issues/6) · [PR #40](https://github.com/monika-mur/medcalc/pull/40) · live in production |
...
**Carried forward, not closed by these:** F-01's follow-ups (F2 numeric scale, F3 GDPR erasure, F9 CI gating, D-01 mirrored grants, the partial-index remark), S-01's (`specialists-tests`, `signed-in-landing`), S-02's (`deferred-tests`, plus the three entries in `review-fixes.md`), S-03's (`visits-tests`, `typecheck-in-ci`, `specialists-page-load-guard`, `inherited-pattern-latents`) and S-04's (`supply-engine-tests`, `timezone-classification`) remain open in their change folders. S-05 adds its own (`deferred-tests`, which explicitly also covers the new INSERT policy — see that file's note on why this one differs from the other three) and widens rather than closes S-04's `timezone-classification`. All five changes are deliberately left unarchived until those queues drain. Two pairs are duplicates and should be collapsed before archiving, because archived folders are read-only: F-01's F9 is S-03's `typecheck-in-ci`, and S-02's "mirror the failed-read empty state into the specialists page" is S-03's `specialists-page-load-guard`.
